...
function Tank(name, connection, tournament, game) {
    this.name = name;
    ...
    return new Promise(function(resolve) {
        self.connection.request('tournaments/' + self.tournament + '/moves', {
            ...
            if (Date.now() < end) {
                ...
                var trajectories = data.outcome.filter(function(n) {
                    return n.name === self.name;
                })[0].hitCoordinates;

                trajectories.angle = angle;
                trajectories.power = power;

                if (trajectories) {
                    self.game.trajectories.push(trajectories);
                }
